...
fluid_synth_all_sounds_off = cfunc(
    "fluid_synth_all_sounds_off", c_int, ("synth", c_void_p, 1), ("chan", c_int, 1)
)
...
fluid_synth_set_midi_router = cfunc(
    "fluid_synth_set_midi_router", None, ("synth", c_void_p, 1), ("router", c_void_p, 1)
)
...
fluid_midi_event_get_channel = cfunc(
    "fluid_midi_event_get_channel", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_control = cfunc(
    "fluid_midi_event_get_control", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_program = cfunc(
    "fluid_midi_event_get_program", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_key = cfunc(
    "fluid_midi_event_get_key", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_type = cfunc(
    "fluid_midi_event_get_type", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_value = cfunc(
    "fluid_midi_event_get_value", c_int, ("evt", c_void_p, 1)
)

fluid_midi_event_get_velocity = cfunc(
    "fluid_midi_event_get_velocity", c_int, ("evt", c_void_p, 1)
)
...
fluid_player_add = cfunc(
    "fluid_player_add", c_int, ("player", c_void_p, 1), ("filename", c_char_p, 1)
)
...
new_fluid_cmd_handler = cfunc(
    "new_fluid_cmd_handler", c_void_p, ("synth", c_void_p, 1), ("router", c_void_p, 1)
)
...
class Synth:
    ...
    def get_setting(self, opt):
        ...
        val = c_int()
        if (
            fluid_settings_getint(self.settings, opt.encode(), byref(val))
            == FLUIDSETTING_EXISTS
        ):
            return val.value
        strval = create_string_buffer(32)
        if (
            fluid_settings_copystr(self.settings, opt.encode(), strval, 32)
            == FLUIDSETTING_EXISTS
        ):
            return strval.value.decode()
        num = c_double()
        if (
            fluid_settings_getnum(self.settings, opt.encode(), byref(num))
            == FLUIDSETTING_EXISTS
        ):
    ...
    def start(self, driver=None, device=None, midi_driver=None, midi_router=None):
        ...
        if midi_router is None:  ## Use fluidsynth to create a MIDI event handler
            ...
            self.custom_router_callback = CFUNCTYPE(c_int, c_void_p, c_void_p)(
                midi_router
            )
            self.midi_driver = new_fluid_midi_driver(
    ...
    def program_info(self, chan):
        ...
        if fluid_synth_get_program is not None:
            ...
            presetnum = c_int()
            fluid_synth_get_program(
                self.synth, chan, byref(sfontid), byref(banknum), byref(presetnum)
            )
    ...
    def set_reverb(self, roomsize=-1.0, damping=-1.0, width=-1.0, level=-1.0):
        ...
        if fluid_synth_set_reverb is not None:
            ...
            return fluid_synth_set_reverb_full(
                self.synth, flags, roomsize, damping, width, level
            )
    ...
    def set_chorus(self, nr=-1, level=-1.0, speed=-1.0, depth=-1.0, type=-1):
        ...
        if fluid_synth_set_chorus is not None:
            ...
            return fluid_synth_set_chorus_full(
                self.synth, set, nr, level, speed, depth, type
            )
    ...
    def play_midi_file(self, filename):
        ...
        if self.custom_router_callback is not None:
            fluid_player_set_playback_callback(
                self.player, self.custom_router_callback, self.synth
            )
